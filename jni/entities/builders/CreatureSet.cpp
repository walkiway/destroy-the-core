...
#define  LOG_TAG    "CreatureSet"

#include "CreatureSet.h"
...
CreatureSet::CreatureSet(stringc worldName, stringc resourceName) :
...
{
  map<stringc, CreatureDefinition*>::Iterator iterator =
      _creatures.getIterator();
  while (!iterator.atEnd())
  {
    map<stringc, CreatureDefinition*>::Node* node = iterator.getNode();
...
void CreatureSet::Add(CreatureDefinition* creatureDefinition)
{
  creatureDefinition->SetWorldName(_worldName);
  _creatures.insert(creatureDefinition->GetName(), creatureDefinition);
}

CreatureDefinition* CreatureSet::GetByName(stringc creatureName)
{
  CreatureDefinition* creatureDef = NULL;
  map<stringc, CreatureDefinition*>::Node* node = _creatures.find(creatureName);

  if (NULL != node)
  {
    creatureDef = node->getValue();
  }
...
stringc CreatureSet::GetWorldName()
{
  return _worldName;
}

stringc CreatureSet::GetResourceName()
{
  return _resourceName;
}

unsigned int CreatureSet::GetNumberOfCreatures()
{
  return _creatures.size();
}

int CreatureSet::GetSpriteWidth()
{
  return spriteWidth;
}

void CreatureSet::SetSpriteWidth(int spriteWidth)
{
  this->spriteWidth = spriteWidth;
}

int CreatureSet::GetSpriteHeight()
{
  return spriteHeight;
}

void CreatureSet::SetSpriteHeight(int spriteHeight)
{
